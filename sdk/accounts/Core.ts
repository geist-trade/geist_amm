 ...
 * @category generated
 */
export class Core implements CoreArgs {
  private constructor(
   ...
   * @returns a tuple of the account data and the offset up to which the buffer was read to obtain it.
   */
  static deserialize(buf: Buffer, offset = 0): [Core, number] {
    return coreBeet.deserialize(buf, offset)
  }
  ...
  serialize(): [Buffer, number] {
    return coreBeet.serialize({
      accountDiscriminator: coreDiscriminator,
      ...this,
    })
  }
   ...
   * depends on them
   */
  static byteSize(args: CoreArgs) {
    const instance = Core.fromArgs(args)
    return coreBeet.toFixedFromValue({
      accountDiscriminator: coreDiscriminator,
      ...instance,
    }).byteSize
  }
  ...
  pretty() {
    return {
      nextPoolId: (() => {
        const x = <{ toNumber: () => number }>this.nextPoolId
 ...
 * @category generated
 */
export const coreBeet = new beet.FixableBeetStruct<
  Core,
  CoreArgs & {
